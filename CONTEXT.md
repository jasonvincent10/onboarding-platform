...
- **Stripe setup — in progress, mid-testing (Aug 2026).** Account created, Stripe Tax enabled, live `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` set and a live-mode webhook destination registered (`checkout.session.completed` → `/api/webhooks/stripe`). Currently verifying via Stripe **test mode** first (separate test-mode secret key + separate test-mode webhook destination, both temporarily swapped into Vercel in place of the live ones) before going live for real — the live values are saved and ready to swap back in once test-mode verification passes. **Security note:** the live secret key was accidentally pasted into chat during setup and was rotated in the Stripe dashboard afterward — confirm before relying on it that the key in place is the rotated one (check rotation history in Stripe's dashboard, not by comparing key text anywhere).
