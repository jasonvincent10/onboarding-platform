...
- **Migration status (9 Sep 2026): 009, 010 and 011 are ALL APPLIED and verified against the live DB.** Verification was a catalog query counting the 9 compliance tables, not an assumption. A first attempt at 009 had rolled back silently: the Supabase SQL editor runs a whole script in one transaction, so a mid-script error reverts everything and reports nothing obvious. The cause was a bare `moddatetime(updated_at)` in CREATE TRIGGER, since 001 creates that extension but never uses it, leaving its schema unproven; 009 now defines a local `set_updated_at()` instead. **Lesson: never treat a quiet run as proof a migration landed. Confirm with a catalog query against information_schema.** Now that real workforce data can exist, avoid full re-runs of 009: the backfill resurrects anyone marked as a leaver, because the unique index only covers active people.
...
- **Migration status: 009 to 012 are applied and verified. `013_corporate_sector.sql` is NOT yet applied** as of 11 Sep 2026 — it adds the `corporate` sector and two new library rows (AML and anti-bribery training), taking the library from 94 to 96. Until it runs, choosing "Corporate and professional services" at signup will fail the sector CHECK constraint.
- **Onboarding feeds compliance**: a template item can name the requirement it satisfies; approving that item creates the compliance record via `lib/compliance/onboarding-sync.ts`. Records land as `pending` unless the document carried a real expiry date, because inferring an issue date from the approval date would show false validity. `decideRecordDates()` is pure and unit-tested.
- **Sector is captured at signup** and seeds that sector's requirements (plus the cross-sector set) into `employer_requirements`, even on the free tier, so Compliance is populated the first time it is opened. Sectors are care, construction, hospitality, logistics, security, corporate and other; "Something else" maps to `cross_sector`. Adding a sector means touching four places: the DB CHECK constraint, `SECTORS`/`SECTOR_LABELS` in `lib/compliance/types.ts`, `SECTOR_ORDER` in `RequirementPicker.tsx`, and the allow-lists in `lib/actions/auth.ts` and `compliance/settings/actions.ts`.
- **Legal pages updated 9 Sep 2026** for compliance records: privacy sections 3 to 8 and DPA section 1. The important correction was section 4, which previously promised an employer only gains access to a category with the employee's consent — untrue for compliance records, which rest on legal obligation and legitimate interest and cannot be withdrawn while employed. Still draft, still needs a solicitor.
...
- ~~Deferred from security session: service-role key rotation~~ — **done (Aug 2026).** No known exposure of the old key, so rather than just rotate it, migrated to Supabase's newer independently-rotatable key system (Project Settings → API → "Publishable and secret API keys" tab) instead of the legacy JWT-based anon/service_role pair. `NEXT_PUBLIC_SUPABASE_ANON_KEY` now holds the `sb_publishable_...` key, `SUPABASE_SERVICE_ROLE_KEY` holds the `sb_secret_...` key — same env var names, app code unchanged, just different value formats (the Supabase client just passes the key through as a bearer credential; it doesn't care about its format). Verified locally (direct Supabase client test: publishable key logs in, secret key bypasses RLS) before touching Vercel, then confirmed working live after redeploy. **Unlike rotating the legacy JWT secret, this did NOT invalidate any active sessions** — the new key system is layered alongside the JWT signing secret, not a replacement for it. Legacy keys are still active in Supabase (not yet disabled) as a safety net; worth disabling them in Settings → API once confident, not urgent. Installed `@supabase/supabase-js`/`@supabase/ssr` versions are quite far behind current releases — not a blocker for this key migration, but worth a separate dependency-upgrade pass sometime.
...
- **Rebrand + employee-flow validation pass (Aug 2026) found and fixed 5 real bugs, missed by the earlier grep-based color sweep:**
  1. `ChecklistView.tsx` had a `console.log` firing on every render of every checklist item (leftover debug code).
  2. The single most important button in the employee checklist ("Upload document" / "Fill in details" / "Read & acknowledge") used a **hardcoded inline `style={{ backgroundColor: '#4f46e5' }}`** — the literal hex of the old indigo-600 — completely invisible to the earlier `teal-`/`indigo-` Tailwind-class rename since it wasn't a class at all. This was the one place the rebrand visibly failed.
  3. The global `*:focus-visible` outline color in `globals.css` was still the old teal hex (`#0d9488`) — every keyboard-focus ring site-wide.
  4. `lib/email/reminder-templates.ts` (deadline reminder + employer escalation emails) still used the old indigo hex for its header/button/link colors.
  5. `lib/email/invite-template.ts` and `team-invite-template.ts` used a dark-neutral header/button (`#0f172a`) rather than the brand purple — updated the background/button uses to violet, deliberately left heading *text* color dark for readability (headings aren't colored brand-purple anywhere else either).
  All fixed to `#7c3aed` (violet-600) / `#ddd6fe` (violet-200 for text-on-violet contrast). **Lesson: a Tailwind-class-based find/replace does not catch hardcoded inline hex styles** — worth a `grep -rn "style={{" | grep "#"` sweep after any future color/brand pass, not just a class-name grep.
- **Employee document upload pipeline verified LIVE, not just by code review (Aug 2026):** created a disposable test employee auth user, signed in with the real publishable key (same as the browser), uploaded a real file to the `employee-documents` bucket, generated a signed URL, and confirmed unauthenticated access is correctly denied — then deleted the test artifacts. Storage RLS is correctly configured. This bucket's policies live only in the Supabase dashboard (see migration 001's own comment: "Run this separately in Supabase Dashboard → Storage") — **not tracked in any migration file**, so there's no version-controlled record of them; if uploads ever start failing, check Storage → Policies in the dashboard directly, nothing to `git blame`.
- **Team member invites — BUILT, not yet live-tested (Aug 2026).** Settings → Team was read-only; now has an invite form + pending-invitations list + revoke button. New migration `004_employer_invitations.sql` (table + RLS + 3 new `audit_action` values) — **had to add explicit `GRANT` statements as a follow-up** after the table was created via SQL Editor, since it came back `permission denied for table employer_invitations` even for the service-role-equivalent key despite RLS being correctly configured — a Postgres privilege issue, not an RLS one. New route `/team-invite?token=...` (added to middleware's `PUBLIC_ROUTES`/`ALWAYS_ACCESSIBLE`, mirroring `/join`) handles both a brand-new signup and an existing-user login, mirrors the onboarding-invite security model exactly (identity always from session, never a param). One deliberate constraint: a user already belonging to a *different* employer is blocked from accepting (`already_on_another_team`), since the rest of the app assumes one employer per user (`dashboard/layout.tsx` etc. all do `.maybeSingle()` on that lookup) — joining a second would put the account in a state nothing else here handles. `role` is stored on invitations/members but not enforced anywhere in the app yet (checked — zero code branches on it), so the invite form doesn't offer a role picker, it'd be misleading. Verified: `tsc --noEmit`, `npm test` (38/38), and `npm run build` all pass; migration confirmed applied and queryable. **Not yet tested: an actual invite email sent, clicked, and accepted end-to-end** — do this before relying on it.
