...
- **Migration 012 (`012_onboarding_compliance_link.sql`) is NOT yet applied** as of 9 Sep 2026 — it adds `compliance_requirement_type_id` to `template_items` and `checklist_items`. Until it runs, the template editor's "Also record this as compliance" picker will error on save.
...
- **Sector is captured at signup** and seeds that sector's requirements (plus the cross-sector set) into `employer_requirements`, even on the free tier, so Compliance is populated the first time it is opened. "Something else" maps to `cross_sector`.
