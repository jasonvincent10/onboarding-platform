...
- **Sign up for Stripe, get live/test API keys**, add `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` to Vercel, and register the webhook endpoint (`/api/webhooks/stripe`, event `checkout.session.completed`) — the code side is ready, this is account setup
